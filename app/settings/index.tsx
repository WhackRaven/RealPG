...
export default function Settings() {
  ...
  const { stats, setLevel, soundEnabled, darkMode, toggleSound, toggleDarkMode, aiMode } = useAppStore();
    ...
    <SafeAreaView style={s.container}>
      ...
      <ScrollView contentContainerStyle={s.content}>
        <View style={s.profileCard}>
          ...
          <TouchableOpacity style={s.avatarButton} onPress={() => router.push('/settings/avatar')}>
        ...
        <View style={s.section}>
          <Text style={s.sectionTitle}>KI-ASSISTENT</Text>
          <SettingItem 
            icon={<Bot color="#9B59B6" size={24} />} 
            label="KI-Assistent" 
            onPress={() => router.push('/settings/ai-models')}
            isDark={isDark}
            right={
              <Text style={{ color: isDark ? '#94A3B8' : '#AFAFAF', fontWeight: '700', fontSize: 14 }}>
                {aiMode === 'cloud' ? 'Cloud' : 'Aus'}
              </Text>
            }
          />
        </View>
